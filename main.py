...
def evaluate(sess, model, name, datanum,char_to_id,tag_to_id, id_to_tag, logger):
    logger.info("evaluate:{}".format(name))
    ner_results,total_loss = model.evaluate(sess, datanum,char_to_id,tag_to_id, id_to_tag,name)
    logger.info("NER total_loss:{:>9.6f}".format(np.mean(total_loss)))
...
def filter_data(data):
    linel=[line[0] for line in data]
    wlen=[len(''.join(w)) for w in linel]
    new_data=[]
    for ind ,wlength in enumerate(wlen):
        if wlength<=400:
...
def train():
    ...
    train_iterlength=5006
    steps_per_epoch = train_iterlength
    with tf.Session(config=tf_config) as sess:
        ...
        for i in range(100):
            ...
            for j in range(steps_per_epoch):
                str_path='/home/lenovo/jianmin/ELMoForManyLangs-master/elmo/NER_train/train_sentence'+str(j+1)+'.npy'
                tag_path='/home/lenovo/jianmin/ELMoForManyLangs-master/elmo/NER_train/train_label'+str(j+1)+'.npy'
                feature_path='/home/lenovo/jianmin/ELMoForManyLangs-master/elmo/NER_train/elmo_result'+str(j+1)+'.npy'
                ...
                for sen in tag_load:
                    ...
                    tags.append(tag)
                batch=convert_feature_tag(str_load,char_to_id,feature_load,tags)
                ...
                if step % FLAGS.steps_check == 0:
                    ...
                    loss = []
            logger.info("NER train_total_loss:{:>9.6f}".format(np.mean(total_train_loss)))
            best = evaluate(sess, model, "dev", 1432,char_to_id, tag_to_id, id_to_tag, logger)
            if best:
                save_model(sess, model, FLAGS.ckpt_path, logger)
            evaluate(sess, model, "test", 716 ,char_to_id,tag_to_id, id_to_tag, logger)
